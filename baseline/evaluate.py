...
Uso:
    python -m baseline.evaluate [--qrels PATH] [--runs PATH [PATH ...]] [--k K]
...
def main():
    ...
    ap.add_argument("--qrels", type=Path, default=Path("../data/qrels.tsv"),
                    help="Caminho para o arquivo qrels.tsv (default: ../../data/qrels.tsv)")
    ap.add_argument("--runs", type=Path, nargs="+",
                    help="Caminhos para os arquivos de run.trec (default: todos os .trec em ../../data/runs/)")
    ap.add_argument("--k", type=int, default=10,
                    help="Valor de k para P@k, R@k, nDCG@k (default: 10)")
    args = ap.parse_args()

    qrels_path = args.qrels
    if not qrels_path.exists():
        print(f"[erro] qrels.tsv não encontrado em {qrels_path}. Por favor, crie-o.", file=sys.stderr)
        sys.exit(1)

    qrels = read_qrels(qrels_path)
    if not qrels:
        print(f"[erro] qrels vazio em {qrels_path}", file=sys.stderr)
        sys.exit(1)

    runs_to_evaluate = args.runs
    if not runs_to_evaluate:
        runs_dir = Path("../../data/runs")
        if not runs_dir.is_dir():
            print(f"[erro] Diretório de runs não encontrado em {runs_dir}", file=sys.stderr)
            sys.exit(1)
        runs_to_evaluate = sorted(list(runs_dir.glob("*.trec")))
        if not runs_to_evaluate:
            print(f"[erro] Nenhum arquivo .trec encontrado em {runs_dir}", file=sys.stderr)
            sys.exit(1)

    evaluate_runs(qrels, runs_to_evaluate, args.k)
